...
//------------------------------------------------------------------------------
vector<pair<int, string>> small_moves[10][200];
...
struct Action {
    ...
    Action(int pidx0, int pidx1, int _vdif0, int _vdif1){
        pidx = pidx0 | (pidx1 << 16);
        vdif0 = _vdif0;
        vdif1 = _vdif1;
    }
    ...
    tuple<int,int,int,int> decode() const {
        return {pidx % (1<<16), pidx / (1<<16), vdif0, vdif1};
    }

    bool operator==(const Action& other) const {
        return pidx == other.pidx && vdif0 == other.vdif0 && vdif1 == other.vdif1;
    }
};
        ...
        void expand(const Evaluator& evaluator, Hash hash, int parent, Selector& selector) {
                ...
                for(auto next_pos : m_target){
                    ...
                    for(auto [p0, s0] : small_moves[i][pos_dif0 + 100]){
                        for(auto [p1, s1] : small_moves[i][pos_dif1 + 100]){
                            ...
                            if(m_target.size() == 1) min_x = max_x;
                            auto cost = evaluator.evaluate() % (1LL << 42) + i * (1LL << 42) + ((long long)max_x - min_x) * (max_y - min_y);
            ...
            for(auto next_pos : m_target){
                ...
                while(cur_pos != next_pos){
                    ++step;
                    int remain0 = next_pos.first - cur_pos.first;
                    int remain1 = next_pos.second - cur_pos.second;
                    if(remain0 * cur_vel.first < 0){
                        ...
                        int remain = abs(remain0);
                        int v = abs(cur_vel.first);
                        bool ok = false;
                        for(int i=1;i>=0;i--){
                    ...
                    if(remain1 * cur_vel.second < 0){
                        ...
                        int remain = abs(remain1);
                        int v = abs(cur_vel.second);
                        bool ok = false;
                        for(int i=1;i>=0;i--){
                            if(remain >= (v + i + 1) * (v + i) / 2){
                ...
                if(m_target.size() == 1) min_x = max_x;
                auto cost = evaluator.evaluate() % (1LL << 42) + step * (1LL << 42) + ((long long)max_x - min_x) * (max_y - min_y);
